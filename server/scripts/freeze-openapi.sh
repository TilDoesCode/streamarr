...
for _ in $(seq 1 60); do
  if curl -sf "$URL/api/v1/health?deep=false" >/dev/null 2>&1; then break; fi
  sleep 0.5
done

mkdir -p "$SERVER_DIR/openapi"
curl -sf "$URL/openapi/v1.json" -o "$TMP/raw.json" || { echo "failed to fetch spec"; tail -20 "$TMP/server.log"; exit 1; }
