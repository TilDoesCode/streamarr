...
public class NzbDocument
{
    ...
    private static async Task<NzbFile> ReadFileAsync(XmlReader reader)
    {
        ...
        if (long.TryParse(dateAttr, out var unixSeconds))
        ...
        while (await reader.ReadAsync().ConfigureAwait(false))
        {
            ...
            {
                await ReadGroupsAsync(reader, file).ConfigureAwait(false);
            }
        }
    ...
    private static async Task ReadGroupsAsync(XmlReader reader, NzbFile file)
    {
        if (reader.IsEmptyElement)
            return;
