...
import { z } from "zod";
import { Loader2, Lock, Play, Plus, SlidersHorizontal, Trash2, X } from "lucide-react";
...
import type { DebugReleaseDto, QualityProfile } from "@/api/types";
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tune how releases are ranked and rejected, then live-preview a query to see the draft
          reorder results before saving (BRIEF §9.1).
        </p>
      </div>
        ...
        {profiles.map((p) => (
          <li key={p.id}>
            ...
            >
              ...
              {p.isDefault && <Lock className="size-3.5 shrink-0" />}
            </button>
          </li>
        ))}
      </ul>
